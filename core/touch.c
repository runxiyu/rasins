...
int main(int argc, char *argv[]) {
	...
	if (param['t']) {
		...
		if (errno) return errprint(argv0, "strptime()", errno);
		if (param['a'])
			times[0].tv_sec = mktime(&time_tm);
		if (param['m'])
			times[1].tv_sec = mktime(&time_tm);
		times[0].tv_nsec = 0; times[1].tv_nsec = 0;
	} else {
		times[0].tv_sec = time(NULL); times[1].tv_sec = time(NULL);
		times[0].tv_nsec = 0; times[1].tv_nsec = 0;
	}
	for (i = 0; i < argc; i++) {
		if (access(argv[i], F_OK) == -1 && errno == ENOENT && !param['c']) {
			/* File does not exist */
			file = creat(argv[i], 
